...
export default async function (m, conn = { user: {} }) {
    ...
    name_user = m.messageStubParameters.map(jid => {
        ...
        return chalk.bold(`${name_info ? name_info : m.idioma_code === 'es' ? 'حد' : 'حد' }`);
    }).join(', ');

    console.log(`
╔═══❖•ೋ° 𝑭𝒂𝒏𝒕𝒂𝒔𝒚𝑩𝒐𝒕-𝑴𝑫 °ೋ•❖═══╗
╞❯ ${chalk.white.bold('بوت:')} ${chalk.cyan.bold('%s')} 
╞❯ ${chalk.white.bold('الوقت:')} ${chalk.black.bgGreen('%s')}
╞❯ ${chalk.white.bold('الإجراء:')} ${m.idioma_code === 'es' ? await formatMessageStubType(m.messageStubType, name_user) : await formaTxtStub(WAMessageStubType[m.messageStubType])}
╞❯ ${chalk.white.bold('المستخدم:')} ${chalk.white('%s')} / ${chalk.bgMagentaBright.bold(user.role.replace(/\*/g, ''))} / بريميوم ⚠︎ ${user?.premiumTime > 0 ? '✔️' : '❌'}
╞❯ ${chalk.white.bold('الموارد:')} ${chalk.yellow('%s%s')}
╞❯ %s
╞❯ ${chalk.white.bold('حجم الرسالة:')} ${chalk.red('%s (%s %sB)')}
╞❯ ${chalk.white.bold('نوع الرسالة:')} ${chalk.bgBlueBright.bold('[%s]')}
╚═════❖•ೋ° Wilmer༝ofc °ೋ•❖═════╝`.trim(),
        ...
        sender,
        m ? '' : '',
        `المستوى ${user.level} / الخبرة ${user.exp} / الألماس ${user.limit} / النقود ${user.money}`,
        (m.chat.includes("@g.us") ? `${chalk.white.bold('جروب:')} ${chalk.magentaBright(`${m.chat} ~${chat}`)}` : `${chalk.white.bold('خاص:')} ${chalk.greenBright(`${m.sender} ~${user.name ? user.name : conn.getName(m.sender)}`)} `),
        filesize,
        filesize === 0 ? 0 : (filesize / 1009 ** Math.floor(Math.log(filesize) / Math.log(1000))).toFixed(1),
        ['', ...'KMGTP'][Math.floor(Math.log(filesize) / Math.log(1000))] || '',
        m.idioma_code === 'es' ? await formatMessageTypes(m.mtype) : await formaTxt(m.mtype) || 'غير محدد'
    );
    ...
    if (/document/i.test(m.mtype)) console.log(`🗂️ ${m.msg.fileName || m.msg.displayName || 'وثيقة'}`);
    else if (/ContactsArray/i.test(m.mtype)) console.log(`👨‍👩‍👧‍👦 ${' ' || ''}`);
    else if (/contact/i.test(m.mtype)) console.log(`👨 ${m.msg.displayName || ''}`);
    else if (/audio/i.test(m.mtype)) {
        const duration = m.msg.seconds;
        console.log(`${m.msg.ptt ? '🎤ㅤ(PTT ' : '🎵ㅤ('}الصوت) ${Math.floor(duration / 60).toString().padStart(2, 0)}:${(duration % 60).toString().padStart(2, 0)}`);
    }
    console.log();
}
...
watchFile(file, () => {
    console.log(chalk.redBright("تم تحديث 'lib/print.j'"));
});
