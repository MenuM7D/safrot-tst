...
if (!text) return conn.reply(m.chat, `*🧚🏼‍♂️ اكتب اسم الاغنيه*\n\n*مثال:*\n#شغل عصام صاصا`, m, {contextInfo: {externalAdReply :{ mediaUrl: null, mediaType: 1, description: null, title: wm, body: '', previewType: 0, thumbnail: img.getRandom(), sourceUrl: redes.getRandom()}}})
const yt_play = await search(args.join(' '))
const texto1 = `📌 *العنوان* : ${yt_play[0].title}\n📆 *نُشر:* ${yt_play[0].ago}\n⌚ *المدة:* ${secondString(yt_play[0].duration.seconds)}`.trim()

await conn.sendButton(m.chat, texto1, botname, yt_play[0].thumbnail, [['الصوت', `${usedPrefix}ytmp3 ${yt_play[0].url}`], ['الفيديو', `${usedPrefix}ytmp4 ${yt_play[0].url}`]], null, null, m)
}
...
await conn.sendButton(m.chat, texto1, botname, yt_play[0].thumbnail, [['الصوت', `${usedPrefix}ytmp3 ${yt_play[0].url}`], ['الفيديو', `${usedPrefix}ytmp4 ${yt_play[0].url}`], ['مزيد من النتائج', `${usedPrefix}yts ${text}`]], null, null, m)
}}
