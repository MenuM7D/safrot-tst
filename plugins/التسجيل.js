...
let handler = async function (m, { conn, text, args, usedPrefix, command }) {
  ...
  // معرف المطور
  let developerID = '201115618853@s.whatsapp.net'

  if (command == 'verify' || command == 'reg' || command == 'verificar') {
    if (m.sender === developerID) return // تخطي التسجيل إذا كان المرسل هو المطور
    if (user.registered === true) throw `*انت مسجل بالفعل 👾*`
    if (!Reg.test(text)) throw `*👾 مش عارف تستخدم الأمر ده؟* استخدمه بالشكل ده: *${usedPrefix + command} الاسم.السن*\n*• مثال:* ${usedPrefix + command} ${name2}.16`
    let [_, name, splitter, age] = text.match(Reg)
    if (!name) throw '*فين الاسم؟*'
    if (!age) throw '*السن مينفعش يبقى فاضي، ضيف السن يا ذكي*'
    if (name.length >= 45) throw '*اي دا يسطا كلو هو انا هطلع ليك بطاقه 😂🦈' 
    age = parseInt(age)
    if (age > 100) throw 'مش للدرجه دي يا عم انت موليد جيل خوفو ولا اي 😂'
    ...
    await conn.reply(m.chat,  `*[ 🧚🏼‍♂️ تسجيل تم ]*
  ...
  if (command == 'unreg') {
    ...
    let sn = createHash('md5').update(m.sender).digest('hex')
    if (args[0] !== sn) throw '😔 *رقم السيريال غلط*'
    global.db.data.users[m.sender].money -= 400
    global.db.data.users[m.sender].limit -= 2
    global.db.data.users[m.sender].exp -= 150
    global.db.data.users[m.sender].joincount -= 2  
